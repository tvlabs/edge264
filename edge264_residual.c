...
void noinline FUNC_TSK(add_idct4x4, int iYCbCr, int qP, i8x16 wS, int DCidx, uint8_t *samples)
{
	...
	i32x4 s8 = set32(8);
	i32x4 d0 = (mul0 * tsk->c_v[0] + s8) >> 4;
	i32x4 d1 = (mul1 * tsk->c_v[1] + s8) >> 4;
	i32x4 d2 = (mul2 * tsk->c_v[2] + s8) >> 4;
	i32x4 d3 = (mul3 * tsk->c_v[3] + s8) >> 4;
	if (DCidx >= 0)
		d0[0] = tsk->c[16 + DCidx];
	...
	if (tsk->samples_clip[iYCbCr][0] == 255) {
		...
		*(int32_t *)(samples + stride * 3) = u[3];
		...
		i16x8 clip = tsk->samples_clip_v[iYCbCr];
...
void FUNC_TSK(add_dc4x4, int iYCbCr, int DCidx, uint8_t *samples) {
	i32x4 x = (set32(tsk->c[16 + DCidx]) + set32(32)) >> 6;
	i32x4 r = packs32(x, x);
	i8x16 zero = {};
	size_t stride = tsk->stride[iYCbCr];
	if (tsk->samples_clip[iYCbCr][0] == 255) {
...
{
	int qP = tsk->QP[iYCbCr];
	size_t stride = tsk->stride[iYCbCr];
	if (tsk->samples_clip[iYCbCr][0] == 255) {
		// loading and scaling
		int div = qP / 6;
		i8x16 zero = {};
		i8x16 *wS = tsk->pps.weightScale8x8_v + (iYCbCr * 2 + mb->f.mbIsInterFlag) * 4;
...
void noinline FUNC_TSK(transform_dc4x4, int iYCbCr)
{
	...
	i32x4 x0 = tsk->c_v[0] + tsk->c_v[1];
	i32x4 x1 = tsk->c_v[2] + tsk->c_v[3];
	i32x4 x2 = tsk->c_v[0] - tsk->c_v[1];
	i32x4 x3 = tsk->c_v[2] - tsk->c_v[3];
	i32x4 x4 = x0 + x1;
	i32x4 x5 = x0 - x1;
	i32x4 x6 = x2 - x3;
	i32x4 x7 = x2 + x3;
	...
	// scale
	unsigned qP = tsk->QP[0]; // FIXME 4:4:4
	i32x4 s32 = set32(32);
	i32x4 LS = set32((tsk->pps.weightScale4x4[iYCbCr][0] * normAdjust4x4[qP % 6][0]) << (qP / 6));
	...
	if (mb->bits[0] & 1 << 5) {
		tsk->c_v[4] = unpacklo64(dc0, dc1);
		tsk->c_v[5] = unpackhi64(dc0, dc1);
		tsk->c_v[6] = unpacklo64(dc2, dc3);
		tsk->c_v[7] = unpackhi64(dc2, dc3);
		
	// ... or prepare for storage in place
	} else {
		size_t stride = tsk->stride[iYCbCr];
		size_t stride3 = stride * 3;
		uint8_t *p = tsk->samples_mb[iYCbCr];
...
void noinline FUNC_TSK(transform_dc2x2)
{
	...
	i32x4 d0 = tsk->c_v[0] + tsk->c_v[1];
	i32x4 d1 = tsk->c_v[0] - tsk->c_v[1];
	...
	unsigned qPb = tsk->QP[1];
	unsigned qPr = tsk->QP[2];
	i32x4 LSb = set32((tsk->pps.weightScale4x4[1 + mb->f.mbIsInterFlag * 3][0] * normAdjust4x4[qPb % 6][0]) << (qPb / 6));
	i32x4 LSr = set32((tsk->pps.weightScale4x4[2 + mb->f.mbIsInterFlag * 3][0] * normAdjust4x4[qPr % 6][0]) << (qPr / 6));
	...
	// ... or prepare for storage in place
		...
		size_t stride = tsk->stride[1];
		size_t stride3 = stride * 3;
		uint8_t *pb = tsk->samples_mb[1];
		uint8_t *pr = tsk->samples_mb[2];
...
#if 0
void FUNC_TSK(transform_dc2x4)
{
	int iYCbCr = (0/*BlkIdx*/ - 8) >> 3; // BlkIdx is 16 or 24
	unsigned qP_DC = 0; //mb->QP[iYCbCr] + 3;
	int w = tsk->pps.weightScale4x4[iYCbCr + mb->f.mbIsInterFlag * 3][0];
	int nA = normAdjust4x4[qP_DC % 6][0];
	__m128i x0 = (__m128i)tsk->c_v[0]; // {c00, c01, c10, c11} as per 8.5.11.1
	__m128i x1 = (__m128i)tsk->c_v[1]; // {c20, c21, c30, c31}
	...
	__m128i *c = (__m128i *)tsk->c_v + 2 + iYCbCr * 2;
	...
	c[1] = _mm_shuffle_epi32(unpackhi64(dc0, dc1), _MM_SHUFFLE(2, 0, 3, 1));
		...
		// loading
		// FIXME scaling
		__m256i d0 = (__m256i)tsk->c_V[0];
		__m256i d1 = (__m256i)tsk->c_V[1];
		__m256i d2 = (__m256i)tsk->c_V[2];
		__m256i d3 = (__m256i)tsk->c_V[3];
		__m256i d4 = (__m256i)tsk->c_V[4];
		__m256i d5 = (__m256i)tsk->c_V[5];
		__m256i d6 = (__m256i)tsk->c_V[6];
		__m256i d7 = (__m256i)tsk->c_V[7];
		...
		*(__m128i *)(q + stride3   ) = _mm256_extracti128_si256(u3, 1);
		...
		// FIXME scaline
		__m128i d0 = (__m128i)tsk->c_v[0];
		__m128i d1 = (__m128i)tsk->c_v[2];
		__m128i d2 = (__m128i)tsk->c_v[4];
		__m128i d3 = (__m128i)tsk->c_v[6];
		__m128i d4 = (__m128i)tsk->c_v[8];
		__m128i d5 = (__m128i)tsk->c_v[10];
		__m128i d6 = (__m128i)tsk->c_v[12];
		__m128i d7 = (__m128i)tsk->c_v[14];
		...
		for (int i = 2;; ) {
			for (int j = 2;; ) {
				...
				tsk->c_v[0] = (v4si)d0;
				d0 = (__m128i)tsk->c_v[1];
				tsk->c_v[2] = (v4si)d1;
				d1 = (__m128i)tsk->c_v[3];
				tsk->c_v[4] = (v4si)d2;
				d2 = (__m128i)tsk->c_v[5];
				tsk->c_v[6] = (v4si)d3;
				d3 = (__m128i)tsk->c_v[7];
				tsk->c_v[8] = (v4si)d4;
				d4 = (__m128i)tsk->c_v[9];
				tsk->c_v[10] = (v4si)d5;
				d5 = (__m128i)tsk->c_v[11];
				tsk->c_v[12] = (v4si)d6;
				d6 = (__m128i)tsk->c_v[13];
				tsk->c_v[14] = (v4si)d7;
				d7 = (__m128i)tsk->c_v[15];
			}
			if (--i == 0)
				break;
			
			// transpose the half matrix going to memory
			__m128i x0 = unpacklo32((__m128i)tsk->c_v[8], (__m128i)tsk->c_v[10]);
			__m128i x1 = unpacklo32((__m128i)tsk->c_v[12], (__m128i)tsk->c_v[14]);
			__m128i x2 = unpackhi32((__m128i)tsk->c_v[8], (__m128i)tsk->c_v[10]);
			__m128i x3 = unpackhi32((__m128i)tsk->c_v[12], (__m128i)tsk->c_v[14]);
			__m128i x4 = unpacklo32(d4, d5);
			__m128i x5 = unpacklo32(d6, d7);
			__m128i x6 = unpackhi32(d4, d5);
			__m128i x7 = unpackhi32(d6, d7);
			tsk->c_v[1] = (v4si)_mm_add_epi32(unpacklo64(x0, x1), set32(32));
			tsk->c_v[3] = (v4si)unpackhi64(x0, x1);
			tsk->c_v[5] = (v4si)unpacklo64(x2, x3);
			tsk->c_v[7] = (v4si)unpackhi64(x2, x3);
			tsk->c_v[9] = (v4si)unpacklo64(x4, x5);
			tsk->c_v[11] = (v4si)unpackhi64(x4, x5);
			tsk->c_v[13] = (v4si)unpacklo64(x6, x7);
			tsk->c_v[15] = (v4si)unpackhi64(x6, x7);
			
			// transpose the half matrix staying in registers
			__m128i x8 = unpacklo32((__m128i)tsk->c_v[0], (__m128i)tsk->c_v[2]);
			__m128i x9 = unpacklo32((__m128i)tsk->c_v[4], (__m128i)tsk->c_v[6]);
			__m128i xA = unpackhi32((__m128i)tsk->c_v[0], (__m128i)tsk->c_v[2]);
			__m128i xB = unpackhi32((__m128i)tsk->c_v[4], (__m128i)tsk->c_v[6]);
		...
		// final residual values
		__m128i r0 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[0], 6), _mm_srai_epi32(d0, 6));
		__m128i r1 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[2], 6), _mm_srai_epi32(d1, 6));
		__m128i r2 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[4], 6), _mm_srai_epi32(d2, 6));
		__m128i r3 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[6], 6), _mm_srai_epi32(d3, 6));
		__m128i r4 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[8], 6), _mm_srai_epi32(d4, 6));
		__m128i r5 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[10], 6), _mm_srai_epi32(d5, 6));
		__m128i r6 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[12], 6), _mm_srai_epi32(d6, 6));
		__m128i r7 = packs32(_mm_srai_epi32((__m128i)tsk->c_v[14], 6), _mm_srai_epi32(d7, 6));
		...
		__m128i clip = set16(tsk->samples_clip[0]); // FIXME 4:4:4
