...
static inline void FUNC_TSK(decode_inter_16x16, i16x8 mvd, int lx)
{
	...
	CALL_TSK(decode_inter, lx * 16, 16, 16);
}
...
static inline void FUNC_TSK(decode_inter_8x16_left, i16x8 mvd, int lx)
{
	...
	CALL_TSK(decode_inter, lx * 16, 8, 16);
}
...
static inline void FUNC_TSK(decode_inter_8x16_right, i16x8 mvd, int lx)
{
	...
	CALL_TSK(decode_inter, lx * 16 + 4, 8, 16);
}
...
static inline void FUNC_TSK(decode_inter_16x8_top, i16x8 mvd, int lx)
{
	...
	CALL_TSK(decode_inter, lx * 16, 16, 8);
}
...
static inline void FUNC_TSK(decode_inter_16x8_bottom, i16x8 mvd, int lx)
{
	...
	CALL_TSK(decode_inter, lx * 16 + 8, 16, 8);
}
 ...
 */
static always_inline void FUNC_TSK(decode_direct_spatial_mv_pred, unsigned direct_flags)
{
	...
	mb->refPic_s[0] = ((i32x4)ifelse_msb(refIdx, refIdx, shuffle8(tsk->RefPicList_v[0], refIdx)))[0];
	mb->refPic_s[1] = ((i32x4)ifelse_msb(refIdx, refIdx, shuffle8(tsk->RefPicList_v[2], refIdx)))[1];
	...
	if (refIdx[0] >= 0)
		CALL_TSK(decode_inter, 0, 16, 16);
	if (refIdx[4] >= 0)
		CALL_TSK(decode_inter, 16, 16, 16);
}
...
static always_inline void FUNC_TSK(decode_direct_temporal_mv_pred, unsigned direct_flags)
{
	...
	// conditional memory storage
	i8x16 lo = shuffle8(tsk->MapPicToList0_v[0], refPicCol);
	i8x16 hi = shuffle8(tsk->MapPicToList0_v[1], refPicCol);
	i8x16 refIdx = ifelse_mask(refPicCol > 15, hi, lo);
	mb->refPic_s[0] = ((i32x4)ifelse_msb(refIdx, refIdx, shuffle8(tsk->RefPicList_v[0], refIdx)))[0]; // overwritten by parse_ref_idx later if refIdx!=0
	mb->refPic_s[1] = ((i32x4)shuffle8(tsk->RefPicList_v[2], (i8x16){}))[0]; // refIdxL1 is 0
	if (direct_flags & 1) {
		mb->refIdx[0] = refIdx[0];
		mb->refIdx[4] = 0;
		mb->mvs_v[0] = temporal_scale(mvCol0, tsk->DistScaleFactor[refIdx[0]]);
	...
	if (direct_flags & 1 << 4) {
		...
		mb->refIdx[5] = 0;
		mb->mvs_v[1] = temporal_scale(mvCol1, tsk->DistScaleFactor[refIdx[1]]);
	...
	if (direct_flags & 1 << 8) {
		...
		mb->refIdx[6] = 0;
		mb->mvs_v[2] = temporal_scale(mvCol2, tsk->DistScaleFactor[refIdx[2]]);
	...
	if (direct_flags & 1 << 12) {
		...
		mb->refIdx[7] = 0;
		mb->mvs_v[3] = temporal_scale(mvCol3, tsk->DistScaleFactor[refIdx[3]]);
	...
	do {
		...
		CALL_TSK(decode_inter, i, widths[type], heights[type]);
		CALL_TSK(decode_inter, i + 16, widths[type], heights[type]);
	} while (direct_flags);
}

static noinline void FUNC_TSK(decode_direct_mv_pred, unsigned direct_flags) {
	if (tsk->direct_spatial_mv_pred_flag) {
		CALL_TSK(decode_direct_spatial_mv_pred, direct_flags);
	} else {
		CALL_TSK(decode_direct_temporal_mv_pred, direct_flags);
	}
}
